"""
train_v3.py — training script for DrivingPlannerV3.

Usage:
    python src/train_v3.py --config configs/v3_baseline.yaml
    python src/train_v3.py --config configs/v3_baseline.yaml --no-wandb
"""
...
def train(cfg: dict, use_wandb: bool) -> None:
    ...
    model = DrivingPlannerV3(
        ...
        causal_layers     = cfg.get("causal_layers",      1),
        K                 = cfg.get("K",                  1),
    ...
    if cfg.get("resume"):
        ...
        log.info("Resumed epoch %d (best ADE %.5f)", start_epoch, best_ade)
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for step, batch in enumerate(train_loader):
            ...
            #     })
            #     log.info("  epoch %d  step %d/%d  loss %.5f", epoch + 1, step + 1, len(train_loader), loss.item())

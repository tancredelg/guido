"""
predict_v4.py — inference and visualisation for DrivingPlannerV4.

Usage:
    uv run src/predict_v4.py --checkpoint checkpoints/best.pth --split val
    uv run src/predict_v4.py --checkpoint checkpoints/best.pth --split test --output submission.csv
    uv run src/predict_v4.py --checkpoint checkpoints/best.pth --split val --visualize
    uv run src/predict_v4.py --checkpoint checkpoints/best.pth --split val --visualize --tta
"""
...
def main():
    ...
    parser.add_argument("--output",     default=None,
                        help="CSV output path (default: submission_v4.csv for test)")
    parser.add_argument("--batch-size", type=int, default=64)
    parser.add_argument("--tta",        action="store_true")
    parser.add_argument("--visualize",  action="store_true",
                        help="Save trajectory + aux predictions PDF")
    parser.add_argument("--vis-output", default="predictions_v4.pdf")
    ...
    if is_test:
        files = _sorted_pkl_files(f"{data_dir}/test_public")
    else:
        files = _sorted_pkl_files(f"{data_dir}/{args.split}")
    ...
    if is_test or args.output:
        out = args.output or "submission_v4.csv"
